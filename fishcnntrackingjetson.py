...
# Inicializações do log da energia consumida pelo sistema durante o processamento
comandopwr=['tegrastats', '--interval', '26.334', '--logfile', '{}Energia.csv'.format(logarquivo)] # Executa o script que faz o monitoramento do consumo de energia via PAPI-RAPL
app=subprocess.Popen(args=comandopwr, stdout=None) # Abre um subprocesso de monitoramento do consumo de energia
monitorjetson = app.pid # Obtêm o número de PID do processo Tegrastats

numeropid = os.getpid() # Obtêm o número de PID do processo atual
# Inicializações do log da carga de CPU para o processamento (detecção e rastreamento)
comandocpu=['python3', '/home/marcelo/Documentos/FishMOT/monitorcpu.py', '--registro={}'.format(logarquivo), '--idprocesso={}'.format(numeropid)] # Executa o script de monitoramento da carga de CPU
monitoracpu=subprocess.Popen(args=comandocpu, stdout=None) # Abre um subprocesso de monitoramento da carga de CPU

print("\nO PID do processo é {}. Agora os dados serão processados...\n".format(numeropid)) # Exibe o PID do processo atual que será monitorado para o usuário
...
with modelo.as_default():
	with tf.Session(graph=modelo) as sessao:
		...
		while True:
			...
			if (totalframes % saltoframe == 0) or manualdetecta == 1:
				# Reseta a detecção manual
				manualdetecta = 0
			...
			for (IDobjeto, centroide) in objetos.items():
				...
				listapontos.append((totalframes, (IDobjeto+1), comecoX, comecoY, (fimX-comecoX), (fimY-comecoY), -1, -1, -1, -1))
...
os.system("sudo kill -15 {}".format(monitorjetson))
